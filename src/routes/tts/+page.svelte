<script lang="ts">
	...
	// Play the next message in queue
	async function processQueue() {
		...
		try {
			const res = await fetch('/api/v1/tts', {
				...
				body: JSON.stringify({ voice: ttsVoice, text: nextMessage })
			});
...
<style>
    ...
    .skip-tts-btn:hover { background-color: #6d28d9; box-shadow: 0 6px 10px rgba(0,0,0,0.25); }
    .skip-tts-btn:active { transform: scale(0.95); }
    .skip-tts-btn:focus { outline: none; box-shadow: 0 0 0 3px rgba(124, 58, 237, 0.5); }
</style>

<audio style="display:none" bind:this={audioEl}></audio>